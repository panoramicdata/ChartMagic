...
Write-Information "Tag $version pushed." -InformationAction Continue
...
Write-Information "Waiting for the release run for $version..." -InformationAction Continue
...
Write-Information "Run: https://github.com/$repoFullName/actions/runs/$runId" -InformationAction Continue
gh run watch $runId --repo $repoFullName --exit-status --interval 20
$runExitCode = $LASTEXITCODE

if ($runExitCode -ne 0) {
	Write-Information "" -InformationAction Continue
	Write-Information "The release run did not succeed: https://github.com/$repoFullName/actions/runs/$runId" -InformationAction Continue
	...
	if ($LASTEXITCODE -eq 0 -and $jobId) {
		...
		if ($LASTEXITCODE -eq 0 -and $annotation) {
			Write-Information "Reason: $annotation" -InformationAction Continue
		}
	}

	Write-Information "" -InformationAction Continue
	Write-Information "Tag $version is pushed but no package was published. Once the cause is fixed:" -InformationAction Continue
	Write-Information "  gh run rerun $runId --repo $repoFullName --failed" -InformationAction Continue
	exit 1
}

Write-Information "Package $version published." -InformationAction Continue
